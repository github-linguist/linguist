...
1. Click on the name of the language in the stats bar to see a list of the files that are identified as that language.
   ...
   [Installing Linguist locally](Installation.md) and running it from the [command line](Usage.md#command-line) will give you accurate results.
1. If you see files that you didn't write in the search results, consider moving the files into one of the [paths for vendored code](../lib/linguist/vendor.yml), or use the [manual overrides](Overrides.md) feature to ignore them.
1. If the files are misclassified, search for [open issues](https://github.com/github/linguist/issues) to see if anyone else has already reported the issue.
   ...
   You can also use the [manual overrides](Overrides.md) feature to correctly classify them in your repository.
...
Keep in mind that the repository language stats are only [updated when you push changes](Design.md#githubcom), and the results are cached for the lifetime of your repository.
...
Correctly detecting the language for the C-family `.h` header files is tough because Linguist detects the languages of files in isolation when analyzing repositories and these header files, especially the smaller ones, can be used across all three languages without using any language-specific content.
...
This will mean you will need to implement an [override](Overrides.md) for some of your header files if you wish for them to be classified as C++ or Objective-C if they do no contain language-specific content.

## My repository isn't showing my language

Linguist does not consider [vendored code](Overrides.md#vendored-code), [generated code](Overrides.md#generated-code), [documentation](Overrides.md#documentation), or `data` (e.g. SQL) or `prose` (e.g. Markdown) languages (as defined by the `type` attribute in [`languages.yml`](../lib/linguist/languages.yml)) when calculating the repository language statistics.
...
If Linguist doesn't know about the language or the extension you're using, consider [contributing](../.github/CONTRIBUTING.md) to Linguist by opening a pull request to add support for your language or extension.
For everything else, you can use the [manual overrides](Overrides.md) feature to tell Linguist to include your files in the language statistics.
...
repositories and thus changes need to be committed as individual files don't show on the file-system.

As a work around you could initialize a temporary Git repository in your directory as demonstrated in this
[script](https://gist.github.com/PuZZleDucK/a45fd1fac3758235ffed9fe0e8aab643).

Alternatively you can run Linguist on individual files, see [here](Usage.md#single-file).
