...
(but see [#2][] for a temporary change in the criteria).
...
This process can help differentiate between, for example, `.h` files which could be either md, json, html, C, C++, or Obj-C.
...
### source of a grammar is really easy:
...
You can the classifier locally with:

```bash
bundle exec script/cross-validation 
```
...
- **@isabelschoeps-thiel** (GitHub Founder)
